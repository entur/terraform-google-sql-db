...
| <a name="input_machine_size"></a> [machine\_size](#input\_machine\_size) | Map of the database instance CPU count (machine\_size.cpu) and memory sizes in MB (machine\_size.memory), or a tier (machine\_size.tier). | `map` | <pre>{<br>  "tier": "db-f1-micro"<br>}</pre> | no |
